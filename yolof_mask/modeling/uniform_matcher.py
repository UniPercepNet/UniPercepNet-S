...
class UniformMatcher(nn.Module):
    ...
    def __init__(self, match_topk: int = 4):
        super().__init__()
    ...
    def forward(self, pred_boxes, anchors, targets):
        ...
        return [
            (torch.as_tensor(i, dtype=torch.int64, device=targets[0].gt_boxes.tensor.device),
             torch.as_tensor(j, dtype=torch.int64, device=targets[0].gt_boxes.tensor.device))
